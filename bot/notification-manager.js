...
exports.fileExists = () => {
    return fs.existsSync('user-notification.json');
}

exports.loadFile = () => {
    return JSON.parse(fs.readFileSync('user-notification.json', 'utf8'));
}

exports.saveFile = (obj) => {
    fs.writeFile('user-notification.json', JSON.stringify(obj), (err) => {
        if(err) throw err;
    });
}